...
export const LEADER_APARTMENTS: LeaderEntry[] = [
  ...
  { region: "경기 고양시 덕양구", name: "DMC한강숲중흥S-CLASS", address: "경기도 고양시 덕양구 덕은동", brand: "중흥S-CLASS", households: 894, complexPk: "41281120439651" },
  // ── 경기 파주시 ─────────────────────────────────────────────
  ...
  // ── 경기 광명시 ─────────────────────────────────────────────
  { region: "경기 광명시", name: "광명역써밋플레이스", address: "경기도 광명시 일직동", brand: "써밋플레이스", households: 1430, complexPk: "41210120361627" },
  // ── 경기 시흥시 ─────────────────────────────────────────────
  ...
  // ── 경기 하남시 ─────────────────────────────────────────────
  { region: "경기 하남시", name: "미사강변골든센트로", address: "경기도 하남시 망월동", brand: "골든센트로", households: 1541, complexPk: "41450120305302" },
  // ── 경기 오산시 ─────────────────────────────────────────────
  // 대장 재선정(2026-07): 기존 "e편한세상오산세교"(지곶동 577·2018년·2050세대)는 84㎡ 실거래 ~3.5억으로
  //   오산 시세 하위권 → 대장 부적합. 호반써밋라포레(궐동·2023.7 입주·867세대)가 84㎡ 실거래 ~5.96억(2025.4),
  //   호가 6.3억으로 오산 완공단지 최고가 = 실질 대장. (근거: hogangnono/한경 실거래 기사)
  // ⚠️ complexPk 미검증(부동산원 CSV/관리자검색 필요) — 임의 입력 금지 규칙에 따라 비워 두고 이름+주소 퍼지매칭에 의존.
  //   확정 시 /admin/verify-leaders 로 complexPk 채울 것.
  { region: "경기 오산시", name: "호반써밋라포레", address: "경기도 오산시 궐동", brand: "호반써밋", households: 867 },
  // ── 경기 평택시 ─────────────────────────────────────────────
  { region: "경기 평택시", name: "평택지제역자이", address: "경기 평택시 동삭동 650", brand: "자이", complexPk: "41220120446607" },
  // ── 경기 안산시 ─────────────────────────────────────────────
  { region: "경기 안산시", name: "그랑시티자이", address: "경기도 안산시 단원구 사동", brand: "자이", households: 6600, complexPk: "41271120400507" },
  // ── 경기 부천시 ─────────────────────────────────────────────
  { region: "경기 부천시", name: "리첸시아중동", address: "경기도 부천시 중동", brand: "리첸시아", households: 572, complexPk: "41192120150170" },
  // ── 경기 군포시 ─────────────────────────────────────────────
