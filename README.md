...
Open `http://127.0.0.1:8000` for the web app, or use `GET /health` and `POST /predict` for direct API access.
...
After training a model and starting the server, open:

```text
http://127.0.0.1:8000
```

The web app provides a Korean apartment-price form that calls `POST /predict` and displays the estimated sale price.
...
If Codex Cloud reports `failed to compute git diff to remote`, see [TROUBLESHOOTING.md](TROUBLESHOOTING.md).
